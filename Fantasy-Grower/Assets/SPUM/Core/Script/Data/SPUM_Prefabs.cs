...
public class SPUM_Prefabs : MonoBehaviour
{
    ...
    public void PopulateAnimationLists()
    {
        ...
        var groupedClips = spumPackages
            ...
            .Where(spumClip =>
                spumClip.HasData && spumClip.UnitType.Equals(UnitType) && spumClip.index > -1
            )
            .GroupBy(spumClip => spumClip.StateType)
            .ToDictionary(group => group.Key, group => group.OrderBy(clip => clip.index).ToList());
        ...
        foreach (var kvp in groupedClips)
        {
            ...
            switch (stateType)
            {
                case "IDLE":
                    IDLE_List.AddRange(
                        orderedClips.Select(clip => LoadAnimationClip(clip.ClipPath))
                    );
