...
public class SkillNodeButton : MonoBehaviour
{
    ...
    [SerializeField] private Text spCostText;
    ...
    public void Initialize(SkillNodeData nodeData, SkillTreePanel ownerPanel)
    {
        ...
        if (skillNameText != null)
            skillNameText.text = nodeData.Skill != null
                ? nodeData.Skill.SkillName + suffix
                : "(없음)";

        if (spCostText != null)
            spCostText.text = nodeData.Skill != null
                ? $"SP: {nodeData.Skill.SPCost}"
                : "SP: 0";
    ...
    public void Refresh(bool isUnlocked, bool canUnlock, bool isSelected)
    {
        ...
        else if (isUnlocked)  background.color = ColorUnlocked;
        else if (canUnlock)   background.color = ColorCanUnlock;
